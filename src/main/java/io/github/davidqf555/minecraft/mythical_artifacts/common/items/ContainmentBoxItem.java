...
public class ContainmentBoxItem extends Item {
    ...
    private static final IFormattableTextComponent LORE = new TranslationTextComponent("item." + MythicalArtifacts.MOD_ID + ".containment_box_item.lore");
    private static final String FILLED_KEY = "item." + MythicalArtifacts.MOD_ID + ".containment_box_item.filled";
    ...
    public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (worldIn instanceof ServerWorld && getPercentFilled(stack) >= 1) {
            if (ArtifactData.get((ServerWorld) worldIn).isFull(ArtifactType.PANDORA_BOX)) {
                stack.getOrCreateChildTag(MythicalArtifacts.MOD_ID).putBoolean("Full", true);
            } else {
